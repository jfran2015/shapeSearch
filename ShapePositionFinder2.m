...
scene_num = 1; % Initialize scene_num

while scene_num <= length(scenes_texture_matrix)
    for i = 1:4
        WaitSecs(0.5);
        running = true;
        ...
        while running == true
            ...
            elseif keyIsDown && keyCode(KbName('LeftArrow'))
                scene_num = max(scene_num - 1, 1);
                WaitSecs(1.0);
            elseif keyIsDown && keyCode(KbName('RightArrow'))
                scene_num = min(scene_num + 1, length(scenes_texture_matrix));
                WaitSecs(1.0);
